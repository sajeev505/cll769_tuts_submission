...
\begin{frame}{Vortex Shedding Frequency}
    \begin{columns}[c]
        ...
        \begin{column}{0.48\textwidth}
            ...
            {\scriptsize \textit{$f \approx 0.10$ Hz (matches $St \approx 0.20$).}}
        \end{column}
    \end{columns}
    \vspace{0.2cm}
    \begin{block}{Frequency Analysis}
    The y-velocity at a point behind the cylinder was monitored. For the laminar case ($Re=272$), the oscillation period $T \approx 11$ s gives a shedding frequency $f \approx 0.091$ Hz ($St \approx 0.183$). For the turbulent case ($Re=2902$), the oscillation period is $T \approx 10$ s, yielding $f \approx 0.10$ Hz. This perfectly matches the theoretical Strouhal number $St \approx 0.20$, showing robust vortex shedding instead of being damped out.
    \end{block}
\end{frame}
