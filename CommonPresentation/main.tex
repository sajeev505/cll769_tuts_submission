...
% --- IIT DELHI LOGO ---
\titlegraphic{\includegraphics[width=2cm]{iitd_logo.png}}
...
\begin{frame}{Problem Statement \& Objective}
    ...
    Validate the CFD predictions for the fully developed velocity profile and pressure drop against analytical solutions (Hagen-Poiseuille flow), verify grid independence, and analyze wall shear stress.
    \end{alertblock}
\end{frame}
...
\begin{frame}{Governing Equations}
    ...
    \vspace{0.2cm}
    \textit{Note: In the axisymmetric cylindrical coordinate system, the azimuthal velocity and $\theta$-derivatives are zero, reducing the equations to 2D $(r, z)$.}
\end{frame}
...
\begin{frame}{Geometry \& Meshing}
    \begin{columns}[c]
        \begin{column}{0.48\textwidth}
            \textbf{Grid Refinement Study:}
            Three meshes were utilized to verify grid independence:
            \bi
                \im \textbf{Coarse:} $50$ mm (x) $\times 10$ mm (y).
                \im \textbf{Medium:} $20$ mm (x) $\times 5$ mm (y) with a smooth transition (bias factor 1.08 towards the wall).
                \im \textbf{Fine:} $5$ mm (x) $\times 2$ mm (y).
            \ei
        \end{column}
        \begin{column}{0.48\textwidth}
            \centering
            ...
            \vspace{0.2cm}
            {\scriptsize \textit{Representative view of the computational grid (Medium mesh).}}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Validation: Velocity Profile \& Pressure Drop}
    \begin{columns}[c]
        \begin{column}{0.5\textwidth}
            \centering
            \includegraphics[width=\linewidth,height=0.55\textheight,keepaspectratio]{tut1_vel_prof.png}\\
            \vspace{0.1cm}
            {\scriptsize \textit{Fully developed radial velocity profile.}}
        \end{column}
        \begin{column}{0.48\textwidth}
            \begin{block}{Velocity Profile}
            \small
            The predicted fully developed profile perfectly matches the analytical parabolic solution:
            \[
            ...
            \small
            The linear pressure drop in the fully developed region matches the Hagen-Poiseuille equation:
            \[
            \frac{\Delta P}{L} = \frac{8 \mu U_{\text{avg}}}{R^2}
            \]
            Grid independence confirmed (Medium and Fine mesh).
            \end{block}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Alternative Approach: Periodic Boundary Conditions}
    \begin{block}{Translationally Periodic BCs}
    If the entrance region development is not of interest, the fully developed flow can be directly simulated using \textbf{Periodic Boundary Conditions}.
    \end{block}
    \vspace{0.2cm}
    \bi
        \im The inlet and outlet are linked as a periodic pair.
        \im Instead of specifying a velocity inlet, a \textbf{specified mass flow rate} is enforced.
        \im The simulation instantly yields the fully developed parabolic velocity profile across the entire domain, saving computational cost by avoiding the long entrance length.
    \ei
\end{frame}
...
\begin{frame}{Geometry \& Meshing}
    \begin{columns}[c]
        \begin{column}{0.5\textwidth}
            ...
            \vspace{0.2cm}
            {\scriptsize \textit{Block-structured computational grid around the cylinder.}}
        \end{column}
        \begin{column}{0.48\textwidth}
            \begin{block}{Mesh Details}
            \bi
                \im A block-structured quadrilateral mesh was generated.
                \im The grid is highly refined near the cylinder walls to capture the boundary layer separation and wake dynamics accurately.
...
\begin{frame}{Velocity Contours: Vortex Street}
    \begin{columns}[c]
        \begin{column}{0.48\textwidth}
            \centering
            \textbf{Laminar ($Re = 272$)} \\
            \includegraphics[width=\linewidth,height=0.4\textheight,keepaspectratio]{vel-contour-272-2000.png}\\
            {\scriptsize \textit{Alternating Von Kármán vortex street.}}
        \end{column}
        \begin{column}{0.48\textwidth}
            \centering
            \textbf{Turbulent ($Re = 2902$)} \\
            \includegraphics[width=\linewidth,height=0.4\textheight,keepaspectratio]{vel-contour-2902-2000.png}\\
            {\scriptsize \textit{Smeared wake due to high turbulent viscosity.}}
        \end{column}
    \end{columns}
    \vspace{0.3cm}
    \begin{block}{Comparison}
    At $Re=272$, distinct alternating vortices are shed. When switching to the Standard $k-\epsilon$ model ($Re=2902$), the vortex shedding is heavily damped and the wake is smeared, a known limitation of the standard RANS formulation due to excessive turbulent viscosity production in the wake.
    \end{block}
\end{frame}
...
\begin{frame}{Vortex Shedding Frequency}
    ...
    The y-velocity at a point behind the cylinder was monitored. For the laminar case, the oscillation period gives the shedding frequency $f$, aligning with the Strouhal number correlation $St = \frac{f D}{U} \approx 0.183$. For the turbulent case, the Standard $k-\epsilon$ model damped out the oscillations entirely, yielding a steady symmetric wake instead.
    \end{block}
\end{frame}
